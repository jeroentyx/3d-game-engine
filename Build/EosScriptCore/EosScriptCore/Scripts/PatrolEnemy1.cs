...
{
    public class PatrolEnemy1 : MonoBase
    {
        ...
        private bool displayCorrupted = false;
        private GameObject CorruptedImage;
        ...
        void OnStart()
        {
            ...
            CorruptedImage = gameObject.FindWithTag("CorruptedImage");
            CorruptedImage.SetActive(false);
            ...
            //corrupted_data_red = gameObject.FindWithTag("corrupted_Data_Red");
            //corrupted_data_blue = gameObject.FindWithTag("corrupted_Data_Blue");
        ...
        void OnUpdate()
        {
            if (sm.CurrentState == ProcessState.Patrol)
            {
                ...
                //Log.LogInfo("Enemey 1 patrolling...");
            }
            ...
            if (sm.CurrentState == ProcessState.Chasing)
            {
                ...
                //Log.LogInfo("Enemy 1 chasing you!");
            }
            ...
            if (displayCorrupted == true)
            {
                CorruptedImage.SetActive(true);
                Log.LogInfo("Corrupted Image is out!");
            }
            if (displayCorrupted == false)
            {
                CorruptedImage.SetActive(false);
                Log.LogInfo("Corrupted Image is not out!");
            }
            if (timer >= 3)
            {
                timerStart = false;
                timer = 0;
                displayCorrupted = false;
            }
        }
